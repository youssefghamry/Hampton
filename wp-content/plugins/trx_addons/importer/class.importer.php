...
class trx_addons_demo_data_importer {
	...
	var $options = array(
		...
		'files'			=> array(									// Demo data files: path to the local file with demo content or URL from external (cloud) server
			'default'	=> array(
				'title'				=> '',						// Installation title ('Light version', 'Portfolio style', etc.)	MUST BE SET IN THE THEME!
		...
		'required_plugins'		=> array(),						// List of the required plugins													MUST BE SET IN THE THEME!
		'plugins_initial_state'	=> 0,							// The initial state of the plugin's checkboxes: 1 - checked, 0 - unchecked		MUST BE SET OR CHANGED IN THE THEME!
		'taxonomies'			=> array(),						// List of the required taxonomies: 'post_type' => 'taxonomy', ...				MUST BE SET OR CHANGED IN THE THEME!
		'additional_options'	=> array(						// Additional options slugs (for export plugins settings)						MUST BE SET OR CHANGED IN THE THEME!
			// WP
			'blogname',
			'blogdescription',
			'posts_per_page',
	...
	function admin_scripts() {
		trx_addons_enqueue_style(  'trx_addons-importer',  trx_addons_get_file_url('importer/importer.css'), array(), null );
		trx_addons_enqueue_script( 'trx_addons-importer', trx_addons_get_file_url('importer/importer.js'), array('jquery'), null, true );
					...
					<div class="trx_importer_demo_set">
						...
						<div class="trx_importer_description trx_importer_description_full<?php if ($this->options['demo_set']!='full') echo ' trx_importer_hidden'; ?>">
							<ol>
                                ...
                                <li><?php echo wp_kses_data(__('ALL EXISTING CONTENTS of your website will be deleted and replaced with the new data!', 'trx_addons')); ?></li>
	...
	function show_exporter_fields($args=array()) {
		...
		<tr>
			...
			<td><a download="<?php echo esc_attr($args['slug']); ?>.txt" href="<?php echo esc_url(trx_addons_get_file_url('importer/export/'.trim($args['slug']).'.txt')); ?>"><?php esc_html_e('Download', 'trx_addons'); ?></a></td>
		</tr>
		<?php
	}
	...
	function exporter() {
		...
		// Export theme mods
		trx_addons_fpc(trx_addons_get_file_dir('importer/export/theme_mods.txt'), serialize($this->prepare_data(get_theme_mods())));
		...
		trx_addons_fpc(trx_addons_get_file_dir('importer/export/theme_options.txt'), serialize($this->prepare_data($options)));
		...
		trx_addons_fpc(trx_addons_get_file_dir('importer/export/widgets.txt'), serialize($this->prepare_data($options)));

		// Export posts
		trx_addons_fpc(trx_addons_get_file_dir('importer/export/posts.txt'), serialize(array(
		...
		$users['usermeta'] = $rows;
		trx_addons_fpc(trx_addons_get_file_dir('importer/export/users.txt'), serialize($users));
	...
	function import_uploads() {
		...
		if ( ($txt = get_option('trx_addons_importer_uploads')) == '' ) {
			if ( ($txt = $this->get_file($this->options['files'][$this->options['demo_type']]['file_with_uploads'])) === false)
				return;
			else
				update_option('trx_addons_importer_uploads', $txt);
		}
		...
		foreach ($files as $file) {
			...
			} else if (substr(trx_addons_get_file_ext($file), 0, 1)==='0' && (int) trx_addons_get_file_ext($file) > 0) {
				if ( ($txt = $this->get_file($file, round(max(0, $counter-1) / count($files) * 100))) === false)
					$need_exit = true;
				else {
					$zip = $this->uploads_dir.'/import_media.tmp';
					trx_addons_fpc($zip, $txt, $file==$files[0] ? 0 : FILE_APPEND);
		...
		if ($result >= 100) update_option('trx_addons_importer_uploads', '');
	...
	function import_dump($slug, $title) {
		...
		if (is_array($data) && count($data) > 0) {
			...
			foreach ($data as $table=>$rows) {
				if ($this->options['debug']) dfl(sprintf(__('Process table "%s"', 'trx_addons'), $table));
	...
	function download_file($fname, $result=0) {
		...
		if (substr($fname, 0, 5)=='http:' || substr($fname, 0, 6)=='https:') {
			...
			$rez = trx_addons_get_file_dir($fname);
		}
		return $rez;
	}
