...
if ( !function_exists( 'trx_addons_widget_recent_news_load_scripts_front' ) ) {
	...
	function trx_addons_widget_recent_news_load_scripts_front() {
		if (trx_addons_is_on(trx_addons_get_option('debug_mode'))) {
			trx_addons_enqueue_style( 'trx_addons-widget_recent_news', trx_addons_get_file_url('widgets/recent_news/recent_news.css'), array(), null );
			trx_addons_enqueue_script( 'trx_addons-widget_recent_news', trx_addons_get_file_url('widgets/recent_news/recent_news.js'), array('jquery'), null, true );
		}
	}
}
...
if ( !function_exists( 'trx_addons_sc_recent_news' ) ) {
	function trx_addons_sc_recent_news($atts, $content=null){	
		...
		while ( $query->have_posts() ) { $query->the_post();
			$post_number++;
			set_query_var('trx_addons_args_recent_news', array(
				'style' => $style,
				'number' => $post_number,
				'count' => $count,
				'columns' => $columns,
			...
			ob_start();
			if (($fdir = trx_addons_get_file_dir('widgets/recent_news/tpl.'.trx_addons_esc($style).'.php')) != '') { include $fdir; }
			else if (($fdir = trx_addons_get_file_dir('widgets/recent_news/tpl.excerpt.php')) != '') { include $fdir; }
...
if (!function_exists('trx_addons_sc_recent_news_add_in_vc')) {
		...
		vc_map( apply_filters('trx_addons_sc_map', array(
				...
				"params" => array(
					...
					array(
						...
						"description" => wp_kses_data( __("Comma separated list of post's ID. If set - parameters above (category, count, order, etc.) are ignored!", 'trx_addons') ),
						"group" => esc_html__('Query', 'trx_addons'),
						"type" => "textfield"
					),
	...
	add_action('after_setup_theme', 'trx_addons_sc_recent_news_add_in_vc', 11);
}
?>