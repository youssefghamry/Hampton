...
class trx_addons_widget_popular_posts extends WP_Widget {
	...
	function widget($args, $instance) {
		...
		for ($i=0; $i<3; $i++) {
			...
			if ($i==0) {			// Most popular
				$args['meta_key'] = 'trx_addons_post_views_count';
				$args['orderby'] = 'meta_value_num';
			} else if ($i==2) {		// Most liked
				$args['meta_key'] = 'trx_addons_post_likes_count';
				$args['orderby'] = 'meta_value_num';
			} else {				// Most commented
			...
			if ( $q->have_posts() ) {
				...
				while ($q->have_posts()) { $q->the_post();
					$post_number++;
					set_query_var('trx_addons_args_widgets_posts', array(
					...
					if (($fdir = trx_addons_get_file_dir('templates/tpl.posts-list.php')) != '') { include $fdir; }
		...
		if ( $tabs[0]['title'].$tabs[0]['content'].$tabs[1]['title'].$tabs[1]['content'].$tabs[2]['title'].$tabs[2]['content'] ) {
			...
			?>
			<div id="<?php echo esc_attr($id); ?>" class="trx_addons_tabs">
				<?php
				if ($tabs_count > 1) {
					?><ul class="trx_addons_tabs_titles"><?php
						foreach ($tabs as $k=>$tab) {
							...
							$id_tab = $id . '_' . $k;
							?><li class="trx_addons_tabs_title"><a href="<?php echo esc_url(trx_addons_get_hash_link('#'.$id_tab.'_content')); ?>"><?php echo esc_html($tab['title']); ?></a></li><?php
						}
					?></ul><?php
				}
				foreach ($tabs as $k=>$tab) {
...
if (!function_exists('trx_addons_sc_widget_popular_posts_add_in_vc')) {
	...
	add_action('after_setup_theme', 'trx_addons_sc_widget_popular_posts_add_in_vc', 11);
}
?>