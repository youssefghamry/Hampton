...
class trx_addons_widget_twitter extends WP_Widget {
	...
	function form( $instance ) {
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id( 'twitter_username' )); ?>"><?php esc_html_e('Twitter Username:', 'trx_addons'); ?><br />(<?php esc_html_e('leave empty if you paste widget code', 'trx_addons'); ?>)</label>
...
if ( !function_exists( 'trx_addons_widget_twitter_load_scripts_front' ) ) {
	...
	function trx_addons_widget_twitter_load_scripts_front() {
		if (trx_addons_is_on(trx_addons_get_option('debug_mode'))) {
			trx_addons_enqueue_style( 'trx_addons-widget_twitter', trx_addons_get_file_url('widgets/twitter/twitter.css'), array(), null );
		}
	}
}
...
if (!function_exists('trx_addons_sc_widget_twitter_add_in_vc')) {
	...
	add_action('after_setup_theme', 'trx_addons_sc_widget_twitter_add_in_vc', 11);
}
?>