...
if ((int)$query->found_posts > 0) {
	...
	?><div class="sc_events sc_events_detailed<?php if ($args['slider']) echo ' swiper-slider-container slider_swiper slider_noresize slider_nocontrols '.((int)$args['slider_pagination'] > 0 ? 'slider_pagination' : 'slider_nopagination'); ?>"<?php
		...
		while ( $query->have_posts() ) { $query->the_post();
			if (($fdir = trx_addons_get_file_dir('api/the-events-calendar/tpl.detailed-item.php')) != '') { include $fdir; }
		}
