...
if (!function_exists('trx_addons_hex2hsb')) {
	function trx_addons_hex2hsb ($hex) {
		return trx_addons_rgb2hsb(trx_addons_hex2rgb($hex));
	}
}
...
// Return truncated string
if (!function_exists('trx_addons_str_decorate')) {
	function trx_addons_str_decorate($str) {
		return str_replace(
					array('{{', '}}', '|'), 
					array('<span class="trx_addons_accent">', '</span>', '<br>'), 
					$str);
	}
}
...
if ( !function_exists('trx_addons_show_layout') ) {
	function trx_addons_show_layout($layout, $before='', $after='') {
		if ($layout != '') {
