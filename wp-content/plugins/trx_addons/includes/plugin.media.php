...
if (!function_exists('trx_addons_add_thumb_size')) {
	function trx_addons_add_thumb_size( $url, $thumb_size, $check_exists = true ) {
		...
		$new_url = trx_addons_remove_protocol( $pi['dirname'] . '/' . $pi['filename'] . '.' . $pi['extension'] );
	...
	function trx_addons_build_slider_layout($args=array(), $images=array()) {
		...
		if (empty($images) || !is_array($images)) {
			...
			foreach ( $images as $k=>$v ) {
				if (empty($v['url']) && !empty($v['image'])) 		$images[$k]['url'] = trx_addons_get_attachment_url($v['image'], $args['thumb_size']);
				if (empty($v['cats']) && !empty($v['subtitle']))	$images[$k]['cats']= $v['subtitle'];
			}
		...
		if (is_array($images) && count($images) > 0) {
			...
			$output .= '<div' . (!empty($args['id']) ? ' id="' . esc_attr($args['id']) . '_outer"' : '')
						. ' class="slider_swiper_outer slider_style_' . esc_attr($args['style'])
							. ' slider_outer_' . esc_attr(trx_addons_is_on($args['controls']) ? 'controls slider_outer_controls_' . esc_attr($args['controls_type']) : 'nocontrols')
							. ' slider_outer_' . esc_attr(trx_addons_is_on($args['pagination']) ? 'pagination' . ' slider_outer_pagination_' . esc_attr($args['pagination_type']) : 'nopagination')
							. ' slider_outer_' . esc_attr(!trx_addons_is_off($args['titles']) ? 'titles_'.$args['titles'] : 'notitles')
							. ' slider_outer_' . esc_attr($args['per_view']==1 ? 'one' : 'multi')
							. '"'
						. '>'
					. '<div' . (!empty($args['id']) ? ' id="' . esc_attr($args['id']) . '_swiper"' : '')
						. ' class="slider_swiper swiper-slider-container' . (!empty($args['class']) ? ' '.esc_attr($args['class']) : '')
							. ' slider_' . esc_attr(trx_addons_is_on($args['controls']) ? 'controls slider_controls_' . esc_attr($args['controls_type']) : 'nocontrols')
							. ' slider_' . esc_attr(trx_addons_is_on($args['pagination']) ? 'pagination' . ' slider_pagination_' . esc_attr($args['pagination_type']) : 'nopagination')
							. ' slider_' . esc_attr(!trx_addons_is_off($args['titles']) ? 'titles_'.$args['titles'] : 'notitles')
							. ' slider_' . esc_attr($args['per_view']==1 ? 'one' : 'multi')
							. ' slider_' . esc_attr($args['noresize']==1 ? 'noresize' : 'resize')
							. ' slider_height_' . esc_attr((int)$args['height']==0 || $args['slides_type']!='bg' ? 'auto' : 'fixed')
			...
			foreach ($images as $image) {
				...
				$video_link = isset($image['video_url']) ? $image['video_url'] : '';
				$video_embed = isset($image['video_embed']) ? str_replace("`", '"', $image['video_embed']) : '';
				if (empty($video_embed) && !empty($video_link)) {
					global $wp_embed;
					if ( is_object($wp_embed) ) {
						$video_embed = $wp_embed->run_shortcode( '[embed]' . trim($video_link) . '[/embed]' );
					}
				}
				if ( !empty($video_embed) && !empty($image['url']) )
					$video_embed = trx_addons_make_video_autoplay($video_embed);
				...
				$output .= '<div ' . (!empty($image['id']) ? ' id="' . esc_attr($image['id']) . '"' : '') 
								...
								. ' style="'
									. ($args['slides_type'] == 'bg' && !empty($image['url']) ? 'background-image:url(' . esc_url($image['url']) . ');' : '')
									//. ((int)$args['height']>0 ? 'min-'.esc_attr(trx_addons_get_css_position_from_values('', '', '', '', '', $args['height'])) : '')
									. (!empty($image['css']) ? esc_attr($image['css']) : '')
									. '"'
								. '>'
								. ($args['slides_type'] == 'bg' || empty($image['url']) ? '' : '<img src="' . esc_url($image['url']) . '" alt="'.(!empty($image['title']) ? esc_attr($image['title']) : '').'">')
								. ($video_embed || $video_link
									? ('<div class="trx_addons_video_player ' . (!empty($image['url']) ? 'with_cover hover_play' : 'without_cover') . '">' 
											. (!empty($image['url']) 
												? '<div class="video_mask"></div><div class="video_hover" data-video="'.esc_attr($video_embed).'"></div>'	
												: '' 
												)
											. '<div class="video_embed video_frame">' . (empty($image['url']) ? trim($video_embed) : '') . '</div>'
										. '</div>')
			...
			$output .= '</div>'
				. ($args['style']=='modern' || $args['titles']=='outside' ? '</div>' : '')
				. (!empty($titles_outside) && $args['titles']=='outside' 
						? '<div class="slider_titles_outside_wrap">'.trim($titles_outside).'</div>' 
						: '')
				. (trx_addons_is_on($args['controls']) 
						? '<div class="slider_controls_wrap">'
								. '<a class="slider_prev swiper-button-prev" href="#">'
									. ($args['style']=='modern' && !empty($args['label_prev']) ? '<span class="slider_controls_label"><span>' . esc_html($prev[0]).'</span>' . (!empty($prev[1]) ? '<span>' . esc_html($prev[1]).'</span>' : '') . '</span>' : '' )
								. '</a>'
								. '<a class="slider_next swiper-button-next" href="#">'
									. ($args['style']=='modern' && !empty($args['label_next']) ? '<span class="slider_controls_label"><span>' . esc_html($next[0]).'</span>' . (!empty($next[1]) ? '<span>' . esc_html($next[1]).'</span>' : '') . '</span>' : '' )
								. '</a>'
							. '</div>' 
						: '')
