...
if (!function_exists('trx_addons_cv_options')) {
	...
	function trx_addons_cv_options($options) {
		...
		if (apply_filters('trx_addons_cv_enable', true)) {
...
if (!function_exists('trx_addons_cv_load')) {
	...
	function trx_addons_cv_load() {
		...
		if ($loaded) return;
		$loaded = true;
		if (apply_filters('trx_addons_cv_enable', trx_addons_is_on(trx_addons_get_option('cv_enable', false, false)))) {
			if (($fdir = trx_addons_get_file_dir('cv/includes/cv.php')) != '') { include_once $fdir; }
			if (trx_addons_get_option('cv_parts[about]') == 1                                                     && ($fdir = trx_addons_get_file_dir('cv/includes/cv.about.php')) != '') { include_once $fdir; }
			if (trx_addons_get_option('cv_parts[resume]') == 1       && defined('TRX_ADDONS_CPT_RESUME_PT')       && ($fdir = trx_addons_get_file_dir('cv/includes/cv.resume.php')) != '') { include_once $fdir; }
			if (trx_addons_get_option('cv_parts[portfolio]') == 1    && defined('TRX_ADDONS_CPT_PORTFOLIO_PT')    && ($fdir = trx_addons_get_file_dir('cv/includes/cv.portfolio.php')) != '') { include_once $fdir; }
			if (trx_addons_get_option('cv_parts[testimonials]') == 1 && defined('TRX_ADDONS_CPT_TESTIMONIALS_PT') && ($fdir = trx_addons_get_file_dir('cv/includes/cv.testimonials.php')) != '') { include_once $fdir; }
			if (trx_addons_get_option('cv_parts[certificates]') == 1 && defined('TRX_ADDONS_CPT_CERTIFICATES_PT') && ($fdir = trx_addons_get_file_dir('cv/includes/cv.certificates.php')) != '') { include_once $fdir; }
		}
	}
}
?>