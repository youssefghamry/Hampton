...
while (have_posts() ) { the_post();
	...
	<article id="post-<?php the_ID(); ?>" <?php post_class( 'trx_addons_cv_single post_type_'.esc_attr(get_post_type()).' itemscope'); ?> itemscope itemtype="http://schema.org/Article">
		...
		$trx_addons_cv_last_link = array(
			'link' => trx_addons_get_cv_page_link(array('section'=>$trx_addons_cv_post_type)),
		...
		<div class="trx_addons_cv_breadcrumbs">
			...
			<a href="<?php echo esc_url(trx_addons_get_cv_page_link(array('section'=>$trx_addons_cv_post_type))); ?>" class="trx_addons_cv_breadcrumbs_item"><?php echo esc_html($trx_addons_cv_post_type_obj->labels->name); ?></a>
			<?php
			if (($trx_addons_cv_taxonomy = apply_filters('trx_addons_filter_cv_get_taxonomy', '', $trx_addons_cv_post_type)) != '') {
				...
				if ($trx_addons_cv_terms && !is_wp_error($trx_addons_cv_terms)) { 
					foreach ($trx_addons_cv_terms as $trx_addons_cv_term) {
						$trx_addons_cv_last_link['link'] = trx_addons_get_cv_page_link(array('section'=>$trx_addons_cv_post_type, 'tab'=>$trx_addons_cv_term->slug));
						$trx_addons_cv_last_link['title'] = $trx_addons_cv_term->name;
						?>
						<a href="<?php echo esc_url(trx_addons_get_cv_page_link(array('section'=>$trx_addons_cv_post_type, 'tab'=>$trx_addons_cv_term->slug))); ?>" class="trx_addons_cv_breadcrumbs_item"><?php echo esc_html($trx_addons_cv_term->name); ?></a>
						<?php
					}
				}
			} else {
		...
		<div class="trx_addons_cv_single_header entry-header">
			...
			<h6 class="trx_addons_cv_single_subtitle"><?php echo trim($trx_addons_cv_meta['subtitle']);?></h6>
		</div><!-- .entry-header -->
