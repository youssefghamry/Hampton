...
if ( !function_exists( 'trx_addons_cv_load_scripts_front' ) ) {
	...
	function trx_addons_cv_load_scripts_front() {
		if (trx_addons_get_value_gp('cv_prn')==1 ) {
			trx_addons_enqueue_style( 'trx_addons-cv.print', trx_addons_get_file_url('cv/css/cv.print.css'), array(), null );
		} else {
			if (trx_addons_is_on(trx_addons_get_option('debug_mode'))) {
				if (trx_addons_is_cv_page()) {
					trx_addons_enqueue_style( 'trx_addons-cv', trx_addons_get_file_url('cv/css/cv.css'), array(), null );
					trx_addons_enqueue_style( 'trx_addons-cv.responsive', trx_addons_get_file_url('cv/css/cv.responsive.css'), array(), null );
					trx_addons_enqueue_script( 'trx_addons-cv', trx_addons_get_file_url('cv/js/cv.js'), array('jquery'), null, true );
				}
			}
			if (trx_addons_is_cv_page()) {
				global $wp_styles;
				trx_addons_enqueue_style( 'trx_addons-cv-ie9', trx_addons_get_file_url('cv/css/cv.ie9.css'), array(), null );
				$wp_styles->add_data( 'trx_addons-cv-ie9', 'conditional', 'lte IE 9' );
				if (!is_customize_preview())
					wp_enqueue_script('jquery-ui-tabs', false, array('jquery', 'jquery-ui-core'), null, true);
				wp_enqueue_script('jquery-ui-accordion', false, array('jquery', 'jquery-ui-core'), null, true);
			}
			trx_addons_enqueue_style( 'trx_addons-cv_buttons', trx_addons_get_file_url('cv/css/cv.buttons.css'), array(), null );
		}
	}
}
