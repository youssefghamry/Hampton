...
$trx_addons_post_date  = sprintf( esc_html__('%s ago', 'trx_addons'), human_time_diff(get_the_time('U')) );
...
if ($trx_addons_show_image) {
	$trx_addons_post_thumb = get_the_post_thumbnail($trx_addons_post_id, trx_addons_get_thumb_size('tiny'), array(
		'alt' => get_the_title()
	));
	if ($trx_addons_post_thumb) $trx_addons_output .= '<div class="post_thumb">' . ($trx_addons_post_link ? '<a href="' . esc_url($trx_addons_post_link) . '">' : '') . ($trx_addons_post_thumb) . ($trx_addons_post_link ? '</a>' : '') . '</div>';
}
