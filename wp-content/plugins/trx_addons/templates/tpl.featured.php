	...
	if ( !$trx_addons_featured_replaced ) {
		...
		if ( $trx_addons_args['singular'] ) {
			...
			if ( has_post_thumbnail() ) {
				...
				?>">
					...
					<a href="<?php the_permalink(); ?>" aria-hidden="true"><?php 
						if (empty($trx_addons_args['thumb_bg']))
							the_post_thumbnail( $trx_addons_thumb_size, array( 'alt' => get_the_title() ) ); 
						else {
					...
					if ($trx_addons_post_format == 'audio') {
						...
						if (!empty($trx_addons_audio)) {
							?><div class="post_audio"><?php echo trim($trx_addons_audio); ?></div><?php
						}
					}
					// Post info
					echo trim($trx_addons_post_info);
					// Hover
				...
				if ( ($trx_addons_output = trx_addons_build_slider_layout(array('thumb_size' => $trx_addons_thumb_size))) != '' ) {
					?><div class="post_featured<?php if (!empty($trx_addons_args['class'])) echo ' '.esc_attr($trx_addons_args['class']); ?>"><?php
						echo trim($trx_addons_output);
						echo trim($trx_addons_post_info);
					?></div><?php
				}
				...
				if (!empty($trx_addons_image)) {
					?><div class="post_featured<?php if (!empty($trx_addons_args['class'])) echo ' '.esc_attr($trx_addons_args['class']); ?>"><a href="<?php the_permalink(); ?>" aria-hidden="true"><?php
						if (empty($trx_addons_args['thumb_bg'])) {
					...
					?></a><?php echo trim($trx_addons_post_info); ?></div><?php
				}
