...
$query_args = trx_addons_query_add_posts_and_cats($query_args, '', TRX_ADDONS_CPT_PORTFOLIO_PT, $args['cat'], TRX_ADDONS_CPT_PORTFOLIO_TAXONOMY);
...
if ((int)$query->found_posts > 0) {
	...
	?><div class="sc_portfolio sc_portfolio_<?php 
		...
		if ($args['slider']) {
			?><div class="sc_portfolio_slider sc_item_slider slides swiper-wrapper"><?php
		} else if ((int)$args['columns'] > 1) {
			?><div class="sc_portfolio_columns sc_item_columns <?php echo esc_attr(trx_addons_get_columns_wrap_class()).($args['type']!='list' ? ' columns_padding_bottom' : ''); ?>"><?php
		} else {
			?><div class="sc_portfolio_content sc_item_content"><?php
		}	

		set_query_var('trx_addons_args_sc_portfolio', $args);
			
		while ( $query->have_posts() ) { $query->the_post();
			if (($fdir = trx_addons_get_file_dir('cpt/portfolio/tpl.'.trx_addons_esc($args['type']).'-item.php')) != '') { include $fdir; }
