<?php
/**
 * The template for displaying team member's page
 *
 * @package WordPress
			...
			if ( has_post_thumbnail() ) {
				?><div class="team_member_featured">
					<div class="team_member_avatar">
						<?php
						the_post_thumbnail( hampton_get_thumb_size('avatar'), array(
									'alt' => get_the_title(),
									'itemprop' => 'image'
									)
								);
				...
				<div class="team_member_details">
					...
					foreach ($meta_box as $k=>$v) {
						if (!empty($v['details']) && !empty($meta[$k])) {
							?><div class="team_member_details_<?php echo esc_attr($k); ?>"><span class="team_member_details_label"><?php echo esc_html($v['title']); ?>: </span><span class="team_member_details_value"><?php echo esc_html($meta[$k]); ?></span></div><?php
						}
					}
					?>
				</div>
