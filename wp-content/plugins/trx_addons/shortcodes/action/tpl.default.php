	...
	foreach ($args['actions'] as $item) {
		...
		?>>
			...
			if (!empty($item['link']) && !empty($item['link_text'])) {
				?><a href="<?php echo esc_url($item['link']); ?>" class="sc_action_item_link sc_button_default2" data-text="<?php echo esc_html($item['link_text']); ?>"><?php echo esc_html($item['link_text']); ?></a><?php
			}
			if (!empty($item['info'])) {
