	...
	public static function register_shortcode($args, $mid_content = null)
	{
		...
		if ($grid_id == '0') {
			...
			if ($content == false) {
				...
				if ($caching == 'true' && 'rand' != $order_by_start) {
			...
			switch ($base->getVar($this->grid_postparams, 'stream-source-type')) {
				case 'twitter':
					...
					if (is_array($tweets) && !empty($tweets)) {
						...
						$this->display_grid_error_msg(true);
						return;
					}
					break;
				...
				case 'facebook':
					...
					if ($base->getVar($this->grid_postparams, 'facebook-type-source') == "album") {
						$photo_set_photos = $facebook->get_photo_set_photos(
							$this->grid_id,
							$base->getVar($this->grid_postparams, 'facebook-access-token'),
							$base->getVar($this->grid_postparams, 'facebook-album'), 
							$base->getVar($this->grid_postparams, 'facebook-count', 10)
						);
					} else {
						$photo_set_photos = $facebook->get_photo_feed(
							$this->grid_id,
							$base->getVar($this->grid_postparams, 'facebook-access-token'),
							$base->getVar($this->grid_postparams, 'facebook-page-id'), 
							$base->getVar($this->grid_postparams, 'facebook-count', 10)
						);
					}
