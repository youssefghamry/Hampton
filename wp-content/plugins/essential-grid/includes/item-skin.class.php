...
class Essential_Grid_Item_Skin
{
	...
	public function init_by_id($id)
	{
		...
		$skin = apply_filters('essgrid_init_by_id', $skin, $id);
		if (!empty($skin)) {
			$this->id = $skin['id'];
			$this->name = $skin['name'];
	...
	public static function update_save_item_skin($data)
	{
		...
		if (isset($data['name'])) {
			...
			return esc_attr__('Invalid name. Name has to be at least 2 characters long.', ESG_TEXTDOMAIN);
		}

		if (isset($data['id'])) {
			if (intval($data['id']) == 0) return esc_attr__('Invalid Item Skin. Wrong ID given.', ESG_TEXTDOMAIN);
		}
		...
		if (!isset($data['params']) || empty($data['params'])) return esc_attr__('No parameters found.', ESG_TEXTDOMAIN);
		...
		if (isset($data['id']) && intval($data['id']) > 0) { 
			...
			if (!empty($check)) {
				return esc_attr__('Item skin with chosen name already exist. Please use a different name.', ESG_TEXTDOMAIN);
			}
			...
			if (!empty($skin)) {
				...
				if ($response === false) return esc_attr__('Item skin could not be changed.', ESG_TEXTDOMAIN);
	...
	public static function delete_item_skin_by_id($data)
	{
		...
		if ($response === false) return esc_attr__('Item Skin could not be deleted', ESG_TEXTDOMAIN);
	...
	public static function star_item_skin_by_id($data)
	{
		...
		if (empty($item_skin)) return esc_attr__('Invalid Skin', ESG_TEXTDOMAIN);
		...
		$response = $wpdb->update($table_name, array('settings' => json_encode($settings)), array('id' => $data['id']));
		if ($response === false) return esc_attr__('Could not change Favorite', ESG_TEXTDOMAIN);
	...
	public static function duplicate_item_skin_by_id($data)
	{
		...
		if (empty($duplicate))
			return esc_attr__('Item Skin could not be duplicated', ESG_TEXTDOMAIN);
		...
		if ($response === false) return esc_attr__('Item Skin could not be duplicated', ESG_TEXTDOMAIN);
	...
	public function output_item_skin($demo = false, $choosen_skin = 0)
	{
		...
		$li_class .= ($is_post) ? ' eg-post-id-' . $post_id : ' eg-post-id-' . $this->item_counter;

		if ($is_post) {
			$li_id = 'eg-' . $this->grid_id . '-post-id-' . $post_id . '_' . rand(0, 10000);
		} else {
			$li_id = 'eg-' . $this->grid_id . '-post-id-' . $this->item_counter . '_' . rand(0, 10000);
		}
		...
		*/
		...
		if ($demo === 'skinchoose') {
			...
			$cltitle = 'title="' . esc_attr__('Select Skin', ESG_TEXTDOMAIN) . '" ';
		}
		...
		if ($demo == 'overview' || $demo == 'skinchoose') {
			...
			if ($demo == 'overview') {
				...
				echo '<a href="javascript:void(0);" title="' . esc_attr__('Mark as Favorite', ESG_TEXTDOMAIN) . '" class="eg-ov-1 eg-overview-button eg-btn-star-item-skin esg-purple eg-tooltip-wrap" id="eg-star-' . $this->id . '"><i class="' . $fav_class . '"></i></a>';
				echo '<a href="' . Essential_Grid_Base::getViewUrl(Essential_Grid_Admin::VIEW_ITEM_SKIN_EDITOR, 'create=' . $this->id) . '" title="' . esc_attr__('Edit Skin', ESG_TEXTDOMAIN) . '" class="eg-tooltip-wrap eg-ov-2 eg-overview-button esg-green "><i class="eg-icon-droplet"></i></a>';
				echo '<a href="javascript:void(0);" title="' . esc_attr__('Duplicate Skin', ESG_TEXTDOMAIN) . '" class="eg-ov-3 eg-overview-button eg-btn-duplicate-item-skin esg-blue eg-tooltip-wrap " id="eg-duplicate-' . $this->id . '"><i class="eg-icon-picture"></i></a>';
				echo '<a href="javascript:void(0);" title="' . esc_attr__('Delete Skin', ESG_TEXTDOMAIN) . '" class="eg-ov-4 eg-overview-button eg-btn-delete-item-skin esg-red eg-tooltip-wrap " id="eg-delete-' . $this->id . '"><i class="eg-icon-trash"></i></a>';
			} elseif ($demo == 'skinchoose') {
				echo '<a href="admin.php?page=essential-grid&view=grid-item-skin-editor&create=' . $this->id . '" class="eg-edit-skin-button eg-overview-button revyellow eg-tooltip-wrap" target="_blank" title="' . esc_attr__('Edit Skin', ESG_TEXTDOMAIN) . '"><i class="eg-icon-tint"></i></a>';
				echo '<input class="eg-tooltip-wrap eg-tooltip-wrap-position " type="radio" value="' . $this->id . '" title="' . esc_attr__('Choose Skin', ESG_TEXTDOMAIN) . '" name="entry-skin"';
				if ($choosen_skin == '-1')
			...
			echo '<a href="javascript:void(0);" class="eg-ov-2 eg-overview-button eg-btn-activate-post-item ' . $vis_icon_color . ' eg-tooltip-wrap" title="' . esc_attr__('Show/Hide from Grid', ESG_TEXTDOMAIN) . '" id="eg-act-post-item-' . $this->post['ID'] . '"><i class="' . $vis_icon . '"></i></a>';
			echo '<a href="' . get_edit_post_link($this->post['ID']) . '" class="eg-ov-3 eg-overview-button esg-purple eg-tooltip-wrap" title="' . esc_attr__('Edit Post', ESG_TEXTDOMAIN) . '" target="_blank"><i class="eg-icon-pencil-1"></i></a>';
			echo '<a href="javascript:void(0);" class="eg-ov-4 eg-overview-button eg-btn-edit-post-item esg-green eg-tooltip-wrap" title="' . esc_attr__('Edit Post Meta', ESG_TEXTDOMAIN) . '" id="eg-edit-post-item-' . $this->post['ID'] . '"><i class="eg-icon-cog"></i></a>';
			echo '</div>' . "\n";
			echo '          </div>' . "\n";
			echo '          <div class="clear"></div>' . "\n\n";
			echo '       </div>' . "\n\n";
			...
			echo '<div class="esg-data-handler esg-display-none" data-exists="' . $custom_layer_elements . '" ></div>'; //has the information on what exists as layers in the skin #3498DB

			echo '<div class="esg-atoolbar esg-atoolbar-transparent">' . "\n";
			echo '          <div class="btn-wrap-item-skin-overview-0">' . "\n";
			echo '<div class="eg-item-skin-overview-name">';

			echo '<div title="' . esc_attr__('Move', ESG_TEXTDOMAIN) . '" class="eg-ov-10 eg-overview-button esg-purple eg-tooltip-wrap esg-cursor-move"><i class="eg-icon-menu"></i></div>';
			echo '<div title="' . esc_attr__('Move one before', ESG_TEXTDOMAIN) . '" class="eg-ov-11 eg-overview-button eg-btn-move-before-custom-element esg-purple eg-tooltip-wrap "><i class="eg-icon-angle-left"></i></div>';
			echo '<div title="' . esc_attr__('Move one after', ESG_TEXTDOMAIN) . '" class="eg-ov-12 eg-overview-button eg-btn-move-after-custom-element esg-purple eg-tooltip-wrap "><i class="eg-icon-angle-right"></i></div>';
			echo '<div title="' . esc_attr__('Move after #x', ESG_TEXTDOMAIN) . '" class="eg-ov-13 eg-overview-button eg-btn-switch-custom-element esg-purple eg-tooltip-wrap "><i class="eg-icon-angle-double-right"></i></div>';

			echo '<div title="' . esc_attr__('Delete Element', ESG_TEXTDOMAIN) . '" class="eg-ov-4 eg-overview-button eg-btn-delete-custom-element esg-red eg-tooltip-wrap "><i class="eg-icon-trash"></i></div>';
			echo '<div title="' . esc_attr__('Duplicate Element', ESG_TEXTDOMAIN) . '" class="eg-ov-3 eg-overview-button eg-btn-duplicate-custom-element esg-blue eg-tooltip-wrap "><i class="eg-icon-picture"></i></div>';
			echo '<div title="' . esc_attr__('Edit Element', ESG_TEXTDOMAIN) . '" class="eg-ov-2 eg-overview-button eg-btn-edit-custom-element esg-green eg-tooltip-wrap "><i class="eg-icon-cog"></i></div>';
						...
						switch ($order) {
							...
							case 'content-image':
								...
								if ($this->lazy_load) {
									...
									$echo_media = '<img src="' . ESG_PLUGIN_URL . 'public/assets/images/300x200transparent.png"' . $small_thumb . ' data-no-lazy="1" data-lazysrc="' . esc_attr($this->media_sources[$order]) . '" alt="' . esc_attr($this->media_sources[$order . '-alt']) . '"' . $img_dim . '>';
								} else {
							...
							case 'content-youtube':
								...
								$video_poster_src = ($this->do_poster_cropping == true) ? esg_aq_resize($video_poster_src, $this->video_sizes[$this->video_ratios['youtube']]['width'], $this->video_sizes[$this->video_ratios['youtube']]['height'], true, true, true) : $video_poster_src;
								$lightbox_thumb = $video_poster_src;
							...
							case 'content-vimeo':
								...
								//if we are masonry, we need to crop the image
								$video_poster_src = ($this->do_poster_cropping == true) ? esg_aq_resize($video_poster_src, $this->video_sizes[$this->video_ratios['vimeo']]['width'], $this->video_sizes[$this->video_ratios['vimeo']]['height'], true, true, true) : $video_poster_src;
								$lightbox_thumb = $video_poster_src;
							...
							case 'content-wistia':
								//if we are masonry, we need to crop the image
								$video_poster_src = ($this->do_poster_cropping == true) ? esg_aq_resize($video_poster_src, $this->video_sizes[$this->video_ratios['wistia']]['width'], $this->video_sizes[$this->video_ratios['wistia']]['height'], true, true, true) : $video_poster_src;
								$lightbox_thumb = $video_poster_src;
							...
							case 'content-html5':
								...
								//if we are masonry, we need to crop the image
								$video_poster_src = ($this->do_poster_cropping == true) ? esg_aq_resize($video_poster_src, $this->video_sizes[$this->video_ratios['html5']]['width'], $this->video_sizes[$this->video_ratios['html5']]['height'], true, true, true) : $video_poster_src;
								$lightbox_thumb = $video_poster_src;
							...
							case 'content-soundcloud':
								//if we are masonry, we need to crop the image
								$video_poster_src = ($this->do_poster_cropping == true) ? esg_aq_resize($video_poster_src, $this->video_sizes[$this->video_ratios['soundcloud']]['width'], $this->video_sizes[$this->video_ratios['soundcloud']]['height'], true, true, true) : $video_poster_src;
								$lightbox_thumb = $video_poster_src;
		...
		if ($link_set_to !== 'none') {
			...
			switch ($link_type_link) {
					...
					if ($demo === false) {
						if ($is_post) {
							...
							if ($get_link == '') {
								...
								$link_wrapper = '<a href="' . $get_link . '"' . $link_target . $link_rel_nofollow . '>%REPLACE%</a>';
							}
						}
					} else {
						$link_wrapper = '<a href="javascript:void(0);"' . $link_target . $link_rel_nofollow . '>%REPLACE%</a>';
				...
				case 'url':
					...
					$link_wrapper = '<a href="' . esc_attr($lurl) . '"' . $link_target . $link_rel_nofollow . '>%REPLACE%</a>';
					break;
				case 'meta':
					if ($demo === false) {
						if ($is_post) {
							...
							if ($meta_link == '') {
								...
								$link_wrapper = '<a href="' . $meta_link . '"' . $link_target . $link_rel_nofollow . '>%REPLACE%</a>';
							}
						} else {
							$meta_key = $base->getVar($this->params, 'link-meta-link', 'javascript:void(0);');
				...
				case 'lightbox':
										...
										case 'html5':
											if (trim($this->media_sources[$order]['mp4']) === '' && trim($this->media_sources[$order]['ogv']) === '' && trim($this->media_sources[$order]['webm'] === '')) {
												...
												if ($lightbox_thumb !== false) {
													$lb_content = '<img class="esg-display-none" src="' . esc_attr($lightbox_thumb) . '" />';
												}
											}
											break;
					...
					$link_wrapper = '<a class="' . $lb_class . '"' . $lb_addition . ' href="' . esc_attr($lb_source) . '" data-thumb="' . esc_attr(esg_aq_resize($this->lightbox_thumbnail, 200)) . '" ' . $lb_caption . $lb_owidth . $lb_oheight . $lb_rel . $lb_data . $lb_featured . $lb_post_title . '>' . $lb_content . '%REPLACE%</a>';
		...
		if ($demo == 'overview' || $demo == 'skinchoose') {
			echo '            <div class="esg-entry-media' . $media_animation . '"' . $media_animation_delay . $media_animation_duration . $media_blur . $data_transition_media . '><img src="' . ESG_PLUGIN_URL . 'admin/assets/images/' . $this->cover_image . '" data-no-lazy="1"></div>' . "\n\n";
		} else {
	...
	public function output_add_more()
	{
		echo apply_filters('essgrid_output_add_more', '<li class="filterall eg-addnewitem-wrapper ui-state-disabled">
			<div class="esg-media-cover-wrapper">
				<div class="esg-entry-media"><img src="' . ESG_PLUGIN_URL . 'public/assets/images/300x200transparent.png" data-no-lazy="1"></div>
				<div class="esg-entry-cover">
					...
					<div class="esg-bottom eg-addnewitem-element-2 esg-transition" data-transition="esg-flipup" data-delay="0.1">' . esc_attr__('CHOOSE YOUR ITEM', ESG_TEXTDOMAIN) . '</div>
	...
	public function insert_layer($layer, $demo = false, $masonry = false, $grid_ids = '', $post_ids = '')
	{
		...
		switch ($link_to) {
				...
				if ($demo === false) {
					if ($is_post) {
						...
						if ($get_link == '') {
							...
							$text = '<a href="' . $get_link . '"' . $link_target . $link_rel_nofollow . '>' . preg_replace('/<a href=\"(.*?)\">(.*?)<\/a>/', "\\2", $text) . '</a>';
						}
					}
				} else {
					$text = '<a href="javascript:void(0);"' . $link_target . $link_rel_nofollow . '>' . $text . '</a>';
			...
			case 'url':
				...
				$text = '<a href="' . esc_attr($lurl) . '"' . $link_target . $link_rel_nofollow . '>' . $text . '</a>';
				break;
				...
				if ($demo === false) {
					if ($is_post) {
						...
						if ($meta_link == '') {
							...
							$text = '<a href="' . $meta_link . '"' . $link_target . $link_rel_nofollow . '>' . $text . '</a>';
						}
					} else {
						...
						if ($get_link == '') {
							...
							$text = '<a href="' . $get_link . '"' . $link_target . $link_rel_nofollow . '>' . $text . '</a>';
						}
					}
				} else {
					$text = '<a href="javascript:void(0);"' . $link_target . $link_rel_nofollow . '>' . $text . '</a>';
			...
			case 'javascript':
				$text = '<a href="javascript:' . esc_attr($base->getVar($layer, array('settings', 'link-type-javascript'), 'void(0);')) . '"' . $link_target . $link_rel_nofollow . '>' . $text . '</a>'; //javascript-link
				break;
			...
			case 'lightbox':
				...
				$text = '<a data-thumb="' . esc_attr(esg_aq_resize($this->lightbox_thumbnail, 200)) . '" href="' . esc_attr($lb_source) . '"' . $lb_addition . $lb_caption . $lb_owidth . $lb_oheight . $lb_rel . $lb_data . $lb_featured . $lb_post_title . '>' . $text . '</a>';
		...
		if ($special_item == 'true' && $special_item_type == 'line-break') {
			//line break element
			echo '              <div class="esg-' . $class . ' ' . $unique_class . ' esg-none esg-clear esg-line-break"></div>' . "\n";
		} elseif (trim($text) !== '') { 
	...
	public function get_post_value($handle, $separator, $function, $meta, $catmax = '-1', $taxonomy = "")
	{
		...
		switch ($handle) {
			...
			case 'link':
				...
				break;
			case 'date':
				$postDate = get_date_from_gmt($base->getVar($this->post, "post_date_gmt"));
	 ...
	 * @return string
	 */
	protected function get_media_attr($sources, $type, $attr)
	{
		$result = '';

		//source can be watermarked, check for original source
		if (isset($sources[$type . '-original'])) {
			$url = $sources[$type . '-original'];
		} else {
			$url = Essential_Grid_Base::getVar($sources, $type, '');
		}
	 ...
	 * @return string
	 */
	protected function get_lightbox_title($demo, $is_post)
	{
		$base = new Essential_Grid_Base();
		$lb_title = '';
		...
		if ('on' === $lightbox_title) {
			...
			switch ($lightbox_title_source) {
				case 'demo':
					$lb_title = esc_attr__('demo mode ' . $lightbox_title_source_orig, ESG_TEXTDOMAIN);
					break;
				case 'title':
					if ($is_post)
	 ...
	 * @return string
	 */
	protected function get_lightbox_description($demo, $is_post)
	{
		$base = new Essential_Grid_Base();
		$lb_description = '';
		...
		if ('on' === $lightbox_description) {
			...
			switch ($lightbox_description_source) {
				case 'demo':
					$lb_description = esc_attr__('demo mode ' . $lightbox_description_source_orig, ESG_TEXTDOMAIN);
					break;
				case 'description':
