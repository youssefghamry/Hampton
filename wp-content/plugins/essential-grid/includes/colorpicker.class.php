...
if (!class_exists('ESGColorpicker')) {
	...
	class ESGColorpicker
	{
		...
		public static function get($val)
		{
			...
			$process = ESGColorpicker::process($val, true);
			return $process[0];
		}
		...
		{
			$val = ESGColorpicker::process($val, true);
			$ar = array();
		...
		public static function convert($color, $opacity = '100')
		{
			...
			if ($color == '') return '';
			if (strpos($color, '[{') !== false || strpos($color, 'gradient') !== false) return ESGColorpicker::get($color);
			...
			if ($opacity == -1 || !$opacity || empty($opacity) || !is_numeric($opacity) || $color == 'transparent' || $opacity === 1 || $opacity === 100) {
				if (strpos($color, 'rgba') === false && strpos($color, '#') !== false) {
					return ESGColorpicker::processRgba(ESGColorpicker::sanitizeHex($color), $opacity);
				} else {
					$color = ESGColorpicker::process($color, true);
					return $color[0];
				}
			}
			...
			if (strpos($color, '#') !== false) {
				...
				$color = ESGColorpicker::rgbValues($color, 3);
				return ESGColorpicker::rgbaString($color[0], $color[1], $color[2], $opacity);
			}
		}
		...
		{
			if (!is_string($clr)) {
				if ($processColor) $clr = ESGColorpicker::sanatizeGradient($clr);
				return array(ESGColorpicker::processGradient($clr), 'gradient', $clr);
				...
				$reversed = ESGColorpicker::reverseGradient($clr);
				return array(ESGColorpicker::processGradient($reversed), 'gradient_css', $reversed);
			} elseif (strpos($clr, '#') !== false) {
				return array(ESGColorpicker::sanitizeHex($clr), 'hex');
		...
		public static function sanatizeGradient($obj)
		{
			...
			for ($i = 0; $i < $len; $i++) {
				...
				$cur['a'] = ESGColorpicker::sanitizeAlpha($cur['a']);
		...
		public static function reverseGradient($str)
		{
			...
			foreach ($colors as $clr) {
				...
				$tpe = ESGColorpicker::process($clr, false);
				if ($tpe[1] === 'hex') {
					$clr = ESGColorpicker::sanitizeHex($clr);
					$clr = ESGColorpicker::processRgba($clr);
				}

				if ($prev && $prev === $clr) continue;
				$prev = $clr;

				$clr = ESGColorpicker::rgbValues($clr, 4);
		...
		public static function easeGradient(&$gradient)
		{
			...
			if (class_exists('ESGColorEasing')) {
				...
				for ($i = 0; $i < $len; $i++) {
					$ar[] = $points[$i];
					ESGColorEasing::insertPoints($points[$i], $points[$i + 1], $ar, $easing, $strength);
				}
		...
		public static function processGradient($obj)
		{
			...
			if (array_key_exists('easing', $obj) && $obj['easing'] !== 'none') {
				ESGColorpicker::easeGradient($obj);
			}
		...
		{
			$values = ESGColorpicker::rgbValues($clr, 3);
			return ESGColorpicker::getRgbToHex($values[0], $values[1], $values[2]);
		}
		...
		{
			$values = ESGColorpicker::rgbValues($clr, 4);
			return ESGColorpicker::getRgbToHex($values[0], $values[1], $values[2]);
		}
		...
		{
			$rgb = ESGColorpicker::rgbValues($val, 4);
		...
		{
			$val = explode('||', $val);
			return ESGColorpicker::convert($val[0], $val[1]);
		}
		...
		public static function rgbToRgba($val)
		{
			...
			$val = ESGColorpicker::rgbValues($val, 4);
			return ESGColorpicker::rgbaString($val[0], $val[1], $val[2], $val[3]);
		...
		public function correctValue($color, $opacity = false)
		{
			...
			if (strpos($color, '[{') === false && strpos($color, 'gradient') === false) {
				...
				return ESGColorpicker::convert($color, $opacity); // legacy conversion
		...
		public static function normalizeColor($color)
		{
			...
			if ($tpe === 'hex') {
				$clr = ESGColorpicker::processRgba($clr, true);
				$processed = true;
			} else if ($tpe === 'rgb') {
				$clr = ESGColorpicker::rgbToRgba($clr);
			} else if ($tpe === 'rgba') {
				$clr = preg_replace('/\s+/', '', $clr);
			...
			if ($processed) $clr = ESGColorpicker::sanitizeRgba($clr);
			return $clr;
		}
		...
		public static function normalizeColors($color)
		{
			...
			if (is_array($color)) {
				$total = count($color);
				for ($i = 0; $i < $total; $i++) $color[$i] = ESGColorpicker::normalizeColor($color[$i]);
			} else {
				$color = ESGColorpicker::normalizeColor($color);
			}
		...
		{
			if ($opacity) {
				$color = ESGColorpicker::rgbaToHex($color);
				$color = ESGColorpicker::trimHex($color);
			} else {
				$opacity = ESGColorpicker::rgbValues($color, 4);
				if ($opacity[3] === '1') {
					$color = ESGColorpicker::rgbaToHex($color);
					$color = ESGColorpicker::trimHex($color);
				}
