...
.#handle# .esg-pagination-button:last-child { 
	border-right: none; 
		...
		foreach ($styles as $style) {
			$handle = 'text-'.$style;
			$skin = Essential_Grid_Navigation::get_essential_navigation_skin_by_handle($handle);
			if (!empty($skin)) {
	...
	protected function _update_grids_3014()
	{
		...
		$grids = $c_grid->get_essential_grids(false, false);
		foreach ($grids as $grid) {
			$grid->params = self::update_grid_desktop_xl($grid->params);
			$response = $wpdb->update($table_name, array('params' => json_encode($grid->params)), array('id' => $grid->id));
		}
	}
	...
	{
		$skins = array('pat-lafontaine', 'grant-fuhr', 'leon-draisaitl', 'uwe-krupp', 'clark-gillies', 'rod-langway', 'ray-bourque');
		$skins_data = array(
			'pat-lafontaine' => array(
...
',
		...
		foreach ($skins as $handle) {
			...
			//update
			Essential_Grid_Navigation::update_create_navigation_skin_css(array('sid' => $skin['id'], 'skin_css' => $skin['css']));
		}
	}
...
.flat-light .esg-navigationbutton.esg-loadmore {
	...
	public static function update_grid_desktop_xl($params)
	{
		...
		$to_update = array('columns','columns-width','columns-height','mascontent-height');
