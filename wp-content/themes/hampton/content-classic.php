...
?><div class="column-1_<?php echo esc_attr($hampton_columns); ?>"><article id="post-<?php the_ID(); ?>" 
	<?php post_class( 'post_item post_layout_classic post_layout_classic_'.esc_attr($hampton_columns).' post_format_'.esc_attr($hampton_post_format) ); ?>
	...
	// Featured image
	hampton_show_post_featured( array( 'thumb_size' => hampton_get_thumb_size(
													strpos(hampton_get_theme_option('body_style'), 'full')!==false 
														? ( $hampton_columns > 2 ? 'big' : 'huge' )
														: (	$hampton_columns > 2
															? ($hampton_expanded ? 'med' : 'small')
															: ($hampton_expanded ? 'big' : 'med')
															)
														)
										) );
