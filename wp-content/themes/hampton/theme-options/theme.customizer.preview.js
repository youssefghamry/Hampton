...
( function( $ ) {
	"use strict";
	var $style = $('#hampton-color-scheme-css'),
		api = wp.customize;

	// Prepare inline styles in the preview window
	if ( $style.length == 0 ) {
		$style = $('head').append( '<'+'style type="text/css" id="hampton-color-scheme-css" />' ).find('#hampton-color-scheme-css');
	}
	...
	api.bind( 'preview-ready', function() {
		...
		api.preview.bind( 'refresh-other-controls', function( obj ) {
			...
			if (id == 'body_style') {
				...
				$('.logo .logo_text').html( hampton_prepare_macros(val) );

			} else if (id == 'blogdescription') {
				$( '.logo .logo_slogan' ).html( hampton_prepare_macros(val) );

			} else if (id == 'copyright') {
				$( '.copyright_text' ).html( hampton_prepare_macros(val) );
