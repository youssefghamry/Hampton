...
jQuery(document).on('action.ready', function(e) {
	...
	if (HAMPTON_STORAGE['menu_hover']=='slide_line' || HAMPTON_STORAGE['menu_hover']=='slide_box') {
		setTimeout(function() {
			"use strict";
			jQuery('#menu_main').spasticNav({
				style: HAMPTON_STORAGE['menu_hover']=='slide_line' ? 'line' : 'box',
				color: HAMPTON_STORAGE['menu_hover_color'],
				colorOverride: false
			});
		}, 500);
	}
...
jQuery(document).on('action.init_hidden_elements', function(e, cont) {
	if (HAMPTON_STORAGE['button_hover'] && HAMPTON_STORAGE['button_hover']!='default') {
		jQuery('button:not(.search_submit):not([class*="sc_button_hover_"]),\
				...
				.hampton_tabs .hampton_tabs_titles li a:not([class*="sc_button_hover_"]),\
				#buddypress a.button:not([class*="sc_button_hover_"])\
				').addClass('sc_button_hover_'+HAMPTON_STORAGE['button_hover']);
		if (HAMPTON_STORAGE['button_hover']!='arrow') {
		...
		jQuery('.sidebar .trx_addons_tabs .trx_addons_tabs_titles li a:not([class*="sc_button_hover_style_"]),\
				.hampton_tabs .hampton_tabs_titles li a:not([class*="sc_button_hover_style_"]),\
		...
		jQuery('.mejs-controls button,\
				...
				').removeClass('sc_button_hover_'+HAMPTON_STORAGE['button_hover']);
